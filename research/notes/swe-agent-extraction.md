...
This is the right pattern for LeVik. The execution interface should be declared data-first and inspected by code, not embedded inside one giant agent prompt.
...
For LeVik, every host action should have optional state probes, such as:
...
- Replace SWE-agent tool bundles with a LeVik host action registry owned by the Go daemon.
...
- any tool surface that is larger than necessary for the first reliable LeVik loop

## Concrete LeVik Impact

LeVik should gain a `HostActionSpec` model with fields like:
